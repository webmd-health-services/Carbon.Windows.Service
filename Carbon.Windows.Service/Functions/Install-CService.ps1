...
function Install-CService
{
    ...
    .EXAMPLE
    Install-CService -Name DeathStar -Path C:\ALongTimeAgo\InAGalaxyFarFarAway\DeathStar.exe -OnFirstFailure RunCommand -FailureRunCommandDelay '0:00:05' -Command 'engage_hyperdrive.exe "Corruscant"' -OnSecondFailure Restart -FailureRestartDelay '00:00:30' -OnThirdFailure Reboot -FailureRebootDelay '00:02:00' -FailureResetPeriod '1.00:00:00'
    ...
    param(
        ...
        # How long to wait before restarting a service after a failure? Default is 1 minute. Only used by `Restart`
        # failure actions. Ignored if there are no `Restart` failure actions.
        [TimeSpan] $FailureRestartDelay = [TimeSpan]::New(0, 1, 0),

        # How long to wait before rebooting a server after a service failure? Default is 1 minute. Only used by `Reboot`
        # failure actions. Ignored if there are on `Reboot` failure actions.
        [TimeSpan] $FailureRebootDelay = [TimeSpan]::New(0, 1, 0),

        # What other services does this service depend on?
        [String[]] $Dependency,

        # The command to run when a service fails, including path to the command and arguments. Only used by
        # `RunCommand` failure actions. Ignored if there are no `RunCommand` failure actions.
        [String] $FailureCommand,

        # How many milliseconds to wait before running the failure command. Default is 0, or immediately. Only used by
        # `RunCommand` failure actions. Ignored if there are no `RunCommand` failure actions.
        [TimeSpan] $FailureRunCommandDelay = [TimeSpan]::Zero,
    ...
    function ConvertTo-FailureActionArg($action)
    {
        if( $action -eq 'Reboot' )
        {
            return "reboot/{0}" -f [int]$FailureRebootDelay.TotalMilliseconds
        }
        elseif( $action -eq 'Restart' )
        {
            return "restart/{0}" -f [int]$FailureRestartDelay.TotalMilliseconds
        }
        elseif( $action -eq 'RunCommand' )
        {
            return 'run/{0}' -f [int]$FailureRunCommandDelay.TotalMilliseconds
        }
    ...
    else
    {
        ...
        {
            # FailureRebootDelay, FailureRestartDelay, and FailureRunCommandDelay used here:
            $varName = "Failure${actionType}Delay"
