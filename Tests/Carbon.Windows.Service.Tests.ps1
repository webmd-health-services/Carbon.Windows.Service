...
BeforeAll {
    Import-Module -Name (Join-Path -Path $PSScriptRoot -ChildPath '..\Carbon.Windows.Service' -Resolve)
    ...
    function ThenUseApprovedVerbs
    {
        ...
        $verbs =
            Get-Command -Module 'Carbon.Windows.Service'|
...
Describe 'Carbon.Windows.Service' {
    It 'should have about help topic' {
        GivenModuleImported
        ThenHelpTopic 'about_Carbon.Windows.Service' -Exists
    }
    ...
    It 'should have a help topic for each command' {
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'Carbon.Windows.Service' -CommandType Function,Cmdlet,Filter))
        {
